...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}nexara-{environment}` (e.g., `nexara-stage`)
...
| `orgName` | string | `nexara` | Organization name (for tags and secret paths) |
| `owner` | string | `nexara` | Repository owner (for tags) |
| `repo` | string | `nexara` | Repository name (for tags and secret paths) |
| `namespace` | string | `nexara` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/nexara/supabase/{environment}`
...
- **Secret path**: `sourceplane/nexara/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/nexara/supabase/stage
```
