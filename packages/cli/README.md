...
This package is internal to the `nexara` monorepo and is not
...
```
nexara login    [--api-url=URL] [--token=BEARER]
nexara logout
nexara whoami
```
...
```
nexara org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
nexara project create <name> [--idempotency-key=KEY]
nexara env create <project-id> <name> [--idempotency-key=KEY]
nexara api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
nexara webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
nexara usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
nexara billing summary
nexara audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`nexara org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `nexara login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/nexara/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/nexara/config.json` (mode 0644, not a secret). Override
both via `NEXARA_CONFIG_DIR` (used by tests).
