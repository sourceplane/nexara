...
describe("resolveConfigDir", () => {
  it("respects NEXARA_CONFIG_DIR override", () => {
    const prev = process.env["NEXARA_CONFIG_DIR"];
    process.env["NEXARA_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["NEXARA_CONFIG_DIR"];
      else process.env["NEXARA_CONFIG_DIR"] = prev;
    }
  });
});
