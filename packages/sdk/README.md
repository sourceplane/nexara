...
nexara TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the nexara control plane API.

## Install (workspace)

This package is internal to the `nexara` monorepo. Add it as a
...
const client = new nexara({
  baseUrl: "https://api.nexara.orun.dev",
  auth: { kind: "bearer", token: process.env["NEXARA_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `nexaraError`. Branch on
...
| `nexaraError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `nexaraError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new nexara({ fetch })`.
