...
// deleted; all wire I/O now goes through the typed `Nexara` resource
...
//   - `createClient` — constructs a `Nexara` against a target, wired
...
//     `Promise<T>` + thrown `NexaraError` into the same shape the
...
import { Nexara, NexaraError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Nexara;
...
export function createClient(target: ApiTarget, token: string | null): Nexara {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Nexara(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `NexaraError` subclasses surface as `{ ok: false }` carrying the
