...
/**
 * The billing entitlement seam, as reached from `nexus-worker`.
 *
 * A near-copy of `projects-worker/src/billing-client.ts` rather than a shared
 * package, and deliberately so: these are two independently deployed Workers,
 * and a shared module would couple their release cycles to save forty lines of
 * fetch-and-validate. The *contract* is shared (`@saas/contracts/billing`),
